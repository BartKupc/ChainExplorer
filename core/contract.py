...
class ContractClassifier:
    ...
    def _fingerprint_from_logs(self, logs):
        ...
        for log in logs:
            ...
            if topic0 in EVENT_SIGNATURES:
                ...
                if any(trex_event in event_name for trex_event in [
                    'ClaimTopicAdded', 'TrustedIssuerAdded', 'Deployed', 
                    'ImplementationAuthoritySet', 'TREXSuiteDeployed', 'IdentityRegistered',
                    'AgentAdded', 'ComplianceAdded', 'TokenBound'
                ]):
                    trex_events.append(event_name)
                elif "Transfer(address,address,uint256)" in event_name:
                    erc20_events.append(event_name)
                elif "TransferSingle" in event_name or "TransferBatch" in event_name:
                    erc1155_events.append(event_name)
                elif "Approval" in event_name and "address,address,uint256" in event_name:
                    erc721_events.append(event_name)
        
        # Classification hierarchy (priority order)
        classification = None
        details = {
            'events_found': list(event_counts.keys()),
            'event_counts': event_counts
        }
        
        if trex_events:
            classification = "ERC-3643 (T-REX)"
            details['trex_events'] = trex_events
        elif erc721_events:
            classification = "ERC-721"
            details['erc721_events'] = erc721_events
        elif erc1155_events:
            classification = "ERC-1155"
            details['erc1155_events'] = erc1155_events
        elif erc20_events:
            classification = "ERC-20"
            details['erc20_events'] = erc20_events
        else:
    ...
    def classify_transaction(self, tx_hash: str) -> str:
        try:
            ...
            # If we can't determine from input, check the logs for T-REX events
            try:
                tx_receipt = self.w3.eth.get_transaction_receipt(tx_hash)
                if tx_receipt and tx_receipt.get('logs'):
                    for log in tx_receipt['logs']:
                        if log.get('topics') and len(log['topics']) > 0:
                            ...
                            # Check if this is a T-REX event
                            if topic0 in EVENT_SIGNATURES:
                                event_name = EVENT_SIGNATURES[topic0]
                                if any(keyword in event_name for keyword in ['T-REX', 'ERC-3643', 'Deployed', 'ClaimTopicAdded', 'IdentityRegistered']):
    ...
    def classify_block(self, block_data):
        ...
        for tx in transactions:
            try:
                ...
                print(f"DEBUG: Classifying transaction {tx_hash}")
                
                # Use full_classify instead of simple classify_transaction
                full_result = self.full_classify(tx_hash=tx_hash)
                print(f"DEBUG: Full result: {full_result}")
                
                if 'transaction_classification' in full_result:
                    tx_type = full_result['transaction_classification']['type']
                    tx_details = full_result['transaction_classification']['details']
                    print(f"DEBUG: Found transaction_classification: {tx_type}")
                    
                    # Store the full classification data for the template
                    tx['classification'] = {
                        'type': tx_type,
                    ...
                    print(f"DEBUG: Added classification to transaction: {tx['classification']}")
                else:
                    tx_type = "Unknown"
                    print(f"DEBUG: No transaction_classification found")
                    tx['classification'] = {
                        'type': 'Unknown',
                        'category': 'Unknown',
                        'priority': 'Low',
                ...
                print(f"DEBUG: Final transaction type: {tx_type}")
                
                if 'Contract Creation' in tx_type:
                    classifications['contract_creations'] += 1
                    print(f"DEBUG: Counted as contract creation")
                elif 'T-REX' in tx_type or 'ERC-3643' in tx_type:
                    classifications['trex_interactions'] += 1
                    print(f"DEBUG: Counted as T-REX interaction")
                elif 'ERC-20' in tx_type:
                    classifications['erc20_transfers'] += 1
                    print(f"DEBUG: Counted as ERC-20 transfer")
                elif 'ERC-721' in tx_type:
                    classifications['erc721_transfers'] += 1
                    print(f"DEBUG: Counted as ERC-721 transfer")
                else:
                    classifications['generic_contracts'] += 1
                    print(f"DEBUG: Counted as generic contract")
            except Exception as e:
                # If transaction classification fails, count as generic
                classifications['generic_contracts'] += 1
                print(f"Warning: Failed to classify transaction {tx.get('hash', 'unknown')}: {e}")
                continue
